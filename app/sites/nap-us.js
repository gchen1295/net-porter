...
async function cleanProduct(product, proxy)
{
  try
  {
    ...
    if(rawSizeData)
    {
      ...
      {
        let atcLink = `https://atc.chewy.xyz/napus?sku=${rawSizeData[j].id}`//`https://www.net-a-porter.com/us/en/api/basket/addskus/${rawSizeData[j].id}.json`
        cleanSizes.push({
          atcLink,
...
async function getAllProductsEPI(proxy)
{
  try
  {
    ...
    let res = await request({
      url: 'https://api.net-a-porter.com/NAP/US/en/1600/0/summaries?visibility=eip-visible&brandIds=1051,1212,1840,2606',
      headers: {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/77.0.3865.90 Safari/537.36'
      },
      proxy: agent,
